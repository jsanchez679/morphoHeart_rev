...
class Organ():
    ...
    def __init__(self, user_name_in:str, stage:str, strain:str, genotype:str, dir_input:str, dir_res:pathlib.WindowsPath):
        ...
        # set_dir_res()
        dir_parent_out = Path(os.path.join(dir_res,'Results_'+self.user_name))
        dir_parent_out.mkdir(parents=True, exist_ok=True)
        
        # # def set_dir_res(self, dir_res):
        # dir_parent_out = os.path.join(dir_res,'Results_'+self.user_name)
...
dir_res = os.path.normpath('D:/Documents JSP/Dropbox/Dropbox_Juliana/PhD_Thesis/Data_ongoing/LS_ongoing/A_LS_Analysis/im_morphoHeart/LS09_F07_V/')
...
npy_stack = np.load(os.path.join(str(organ1.dir_res), 'stacks_npy','LS09_F07_V_DS_1100_s3_ch0_all.npy'))
rotateZ_90 = True
...
m_Myoc.create_mesh(npy_stack,resolution, extractLargest, rotateZ_90=rotateZ_90)
